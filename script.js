const jogadores = [
  ...
  // Função para exibir os jogadores na página inicial
  ...
  function armazenarDetalhesJogador(jogador) {
    // Armazenar os detalhes do jogador no armazenamento local (localStorage)
    localStorage.setItem('detalhesJogador', JSON.stringify(jogador));
  }
  
  // Função para exibir os detalhes do jogador na página de detalhes
  function exibirDetalhesJogador() {
    const detalhesJogador = JSON.parse(localStorage.getItem('detalhesJogador'));
  
    const elementoDetalhesJogador = document.getElementById('detalhes_jogador');
  
    const imagem = document.createElement('img');
    imagem.src = detalhesJogador.imagem;
  
    const nome = document.createElement('h3');
    nome.textContent = detalhesJogador.nome;
  
    const listaDetalhes = document.createElement('ul');
  
    const posicao = criarItemDetalhe('Posição', detalhesJogador.posicao);
    const elenco = criarItemDetalhe('Elenco', detalhesJogador.elenco);
    const nascimento = criarItemDetalhe('Nascimento', detalhesJogador.nascimento);
    const nomeCompleto = criarItemDetalhe('Nome Completo', detalhesJogador.nome_completo);
    const altura = criarItemDetalhe('Altura', detalhesJogador.altura);
    const descricao = criarItemDetalhe('Descrição', detalhesJogador.descricao);
  
    listaDetalhes.appendChild(posicao);
    listaDetalhes.appendChild(elenco);
    listaDetalhes.appendChild(nascimento);
    listaDetalhes.appendChild(nomeCompleto);
    listaDetalhes.appendChild(altura);
    listaDetalhes.appendChild(descricao);
  
    elementoDetalhesJogador.appendChild(imagem);
    elementoDetalhesJogador.appendChild(nome);
    elementoDetalhesJogador.appendChild(listaDetalhes);
  }
  ...
  // Verificar se estamos na página de detalhes e chamar a função correspondente
...
function pesquisarJogador() {
  ...
  for (let i = 0; i < jogadores.length; i++) {
    const jogador = jogadores[i];
    if (jogador.nome.toLowerCase().includes(query)) {
      armazenarDetalhesJogador(jogador);
      window.location.href = 'detalhes.html';
      return;
    }
